...
var Screen = (function() {
  ...
  Screen.prototype.load = function load() {
    ...
    self.redis.get('screen:' + self.token, function(err, res) {
      if (err) {
        self.error(500, 'Screen could not be loaded from redis.');
        return;
      }
  ...
  Screen.prototype.save = function save() {
    ...
    self.redis.set('screen:' + self.token, JSON.stringify(data), function(err, res) {
      if (err) {
