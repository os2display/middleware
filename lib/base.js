...
var Base = (function() {
  ...
  var Base = function() {
    ...
    if (self.config.get('log')) {
      self.logger = new Log('info', fs.createWriteStream(config.get('log'), {'flags': 'a'}));
    }

    // Connect to redis server.
    var rediesConf = self.config.get('redis')
    self.redis = require("redis").createClient(rediesConf.port, rediesConf.host, { 'auth_pass': rediesConf.auth });
    self.redis.on('error', function (err) {
      self.error(err);
    });
    self.redis.on("connect", function (err) {
      if (self.config.get('debug')) {
        console.log('Connected to redis server at: ' + rediesConf.host);
      }
    });
  }
